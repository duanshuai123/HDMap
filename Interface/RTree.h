...
#define RTMin std::min
#define RTMax std::max
...
public:
  ...
  //by duans
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& vecResult) const;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::vector<DATATYPE>& vecResult) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& vecResult) const
{
  #ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, vecResult);
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(const Rect* a_rectA, const Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
    newRect.m_min[index] = RTMin(a_rectA->m_min[index], a_rectB->m_min[index]);
    newRect.m_max[index] = RTMax(a_rectA->m_max[index], a_rectB->m_max[index]);
  }
...
// Search in an index tree or subtree for all data retangles that overlap the argument rectangle.//by duans
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::vector<DATATYPE>& vecResult) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, vecResult))
        {
          // The callback indicated to stop searching
          return false;
        }
  ...
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        DATATYPE& id = a_node->m_branch[index].m_data;
        ++a_foundCount;
        vecResult.push_back(id);//push back the target
      }
    }
  }
