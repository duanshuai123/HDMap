...
void geoAlgorithm::DistanceofPointToPolyLine(const Vector3d& Pt,const vector<Vector3d>&  vecLine,
...
void geoAlgorithm::DistanceOfPointToSegment(const Vector3d& PtA,const Vector3d& PtB,
                                            const Vector3d& PtS, double& dDis,Vector3d& PtNearest)
{
    //A-B A-C
    Vector3d  AB;
    ...
    double AS_M = NormalizeValue(AS);
    double AB_M = NormalizeValue(AB);
...
//求向量的模
double geoAlgorithm::NormalizeValue(Vector3d v)
{
    return sqrt( v.x()*v.x() + v.y()*v.y() + v.z()*v.z() );
}
    
double geoAlgorithm::DisTance(const vector<Vector3d>& vecPts)
{
    double dDIs =0;
    for(int i = 0;i<vecPts.size()-1;i++)
    {
        dDIs += DisTance(vecPts[i],vecPts[i+1]);
    }
    return dDIs;
}
    
double geoAlgorithm::DisTance(Vector3d pt1,Vector3d pt2)
{
